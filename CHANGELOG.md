...
- Backup file collision when `fix` and `clean` run in sequence on same session — `fix --apply` and `rewire --apply` now clean stale `.bak` files after success
- `CleanAll` and `CleanLive` finalize step no longer hard-fails if `.bak.orig` is missing
