...
**Cross-session**
- Project reasoning graph with structural edges
- Decision conflict detection across sessions
- Continuity index (re-explanation tax measurement)
- Project memory synthesis from session history
- CLAUDE.md sync from distilled decisions

**See also:** [VectorPad](https://github.com/ppiankov/vectorpad) — pre-flight vector control panel (TUI).
...
| 4. Operational control | Complete | Federated project identity, session search, watch mode, smart watch, cost alerts, savings attribution, per-model cost, TUI responsive columns, TUI tabbed detail, vim navigation + search, active dashboard, status line telemetry, expert hygiene, session analytics, decision economics (CPD/TTC/CDR), onboarding polish, input purity score, session integrity guard, [Vector Control panel](https://github.com/ppiankov/vectorpad) |
| 5. Reasoning memory | Complete | Project reasoning graph, conflict detection, project memory synthesis, CLAUDE.md sync, decision lineage |
| 6. Reasoning metrics | Complete | Repo token budget, reasoning half-life, reasoning efficiency map, flight recorder, reasoning diff, throughput metrics |
| 7. Security | Complete | Vector injection detection, cross-project bond, signal-first status line |
...
| [How-To Guide](docs/howto.md) | Fix "session not found", rename folders, auto-checkpoint, preserve decisions, fix Mac errors, session recovery, hooks setup |
